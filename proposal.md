...
| **E. Data Product Quality with Checkup** | Build a skill that wraps the Checkup CLI to generate quality reports on a simulated data product, then iteratively improve it based on findings. | Working `/checkup` skill + improved data product | CLI tool integration, iterative agent loops, output parsing |
