...
1. Authenticate the Conveyor CLI (opens a browser):
   ...
   conveyor auth login
   ```
2. Smoke-test the connection — this is the recipe your skill will encode:
   ```bash
...
3. A skill is just a folder under `.claude/skills/` — this track ships two
...
1. Poke at the environment first: `af dags list`, `af runs list --dag-id …`
   (with the env vars from the smoke test).
2. Open `.claude/skills/airflow-ops/SKILL.md` and work through the TODOs —
   ...
   **dynamic context** (`` !`command` ``), encode the auth recipe (short-lived
   tokens!), map intents to `af` commands, and add a guardrail for
...
How does the dbt build actually get scheduled? `demo/dags/workshop_dbt.py`
shows the Conveyor idiom: `ConveyorDbtTaskFactory` reads the dbt manifest and
generates **one Airflow task per dbt model**. The facilitator deployed it with
`conveyor deploy` — same dbt project as `../../data`, now with a schedule, a
container, and an IAM role. That's the gap between "runs on my laptop" and
"runs in production".
